...
struct MinimapColor
{
  ...
  constexpr MinimapColor() : red(0), green(0), blue(0), alpha(0xFF), rgba(0x000000FF)
  {
  }
};
...
{
  constexpr ConstexprColors() : colors()
  {
    for (int i = 0; i < 256; ++i)
    {
      uint8_t r = i / 36 * 51;
  ...
  constexpr const MinimapColor &operator[](std::size_t i) const { return colors[i]; }
