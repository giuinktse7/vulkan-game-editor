        ...
        while (buffer.peek() != OTBM::Token::Start)
        {
            ...
            switch (static_cast<OTBM::NodeAttribute>(token))
            {
                ...
                case OTBM::NodeAttribute::ExternalHouseFile:
                    ...
                    break;
                default:
                    logWarning("Unknown map header node: " + std::to_string(static_cast<int>(token)));
            }
        }
                ...
                while (buffer.peek() != OTBM::Token::End)
                {
                    ...
                    switch (nodeType)
                    {
                        case OTBM::Node_t::Item:
                            ...
                            break;
                        default:
                            return "[deserializeTileArea] Unknown OTBM::Node_t: " + std::to_string(to_underlying(nodeType));
                    }
...
std::optional<std::string> OTBM::OTBM1Deserializer::deserializeItemAttributes(Item &item)
{
    while (buffer.peek() != OTBM::Token::End)
    {
        ...
        switch (static_cast<OTBM::NodeAttribute>(attribute))
        {
            ...
            default:
                ABORT_PROGRAM("Unknown attribute: " + std::to_string(attribute));
        }
    }
                ...
                for (uint16_t i = 0; i < amount; ++i)
                {
                    ...
                    switch (attributeTypeId)
                    {
                        ...
                        case OTBM::AttributeTypeId::Double:
                            ...
                            break;
                        default:
                            return "Unknown OTBM::AttributeTypeId: " + std::to_string(to_underlying(attributeTypeId));
                    }
                }
